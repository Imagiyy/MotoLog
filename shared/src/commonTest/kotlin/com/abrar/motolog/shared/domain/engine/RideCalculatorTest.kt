...
class RideCalculatorTest {
    ...
    fun stationaryJitter_fiveMinutesAddsUnderTenMeters() {
        val points = FixturePoints.stationaryJitter()
    ...
    fun highwayRide_sustainedSpeedComputesExpectedDistanceAndAverages() {
        val points = FixturePoints.highwayRide()
    ...
    fun cityRide_urbanRouteWithTrafficLightsCalculatesAccurately() {
        val points = FixturePoints.cityRide()
        ...
        assertTrue(
            ...
            "Moving time must be approx 765s (was ${stats.movingTimeMs}ms)"
        )
        assertTrue(
            stats.stoppedTimeMs in 125_000L..145_000L,
            "Stopped time must be approx 135s (was ${stats.stoppedTimeMs}ms)"
        )
    ...
    fun maxSpeed_unconfirmedNoisySpikeDoesNotSetMaxSpeed() {
        ...
        assertEquals(60.0, calculator.stats.maxSpeedKmh, 0.5, "Single unconfirmed noisy point must not set max speed")
        ...
        assertEquals(100.0, calculator.stats.maxSpeedKmh, 0.5, "Confirmed speed across 2 readings should update max speed")
    }
    ...
    fun avgMovingSpeed_doesNotIncreaseWhileStoppedAtTrafficLight() {
        ...
        assertEquals(movingAvgBeforeStop, movingAvgAfterStop, 0.01, "Moving average speed must not increase while stopped")
    ...
    fun startConfirmation_suppressesSpeedAndDistanceUnder100mAndCreditsOnCrossingThreshold() {
        ...
        assertEquals(0.0, calculator.stats.currentSpeedKmh, 0.001, "Speed must remain 0 under 100m gate")
        assertEquals(0.0, calculator.stats.totalDistanceMeters, 0.001, "Distance must remain 0 under 100m gate")
        assertEquals(0L, calculator.stats.movingTimeMs)
        ...
        assertEquals(110.0, calculator.stats.totalDistanceMeters, 2.0, "Full displacement must be credited once threshold is reached")
        assertEquals(28.8, calculator.stats.currentSpeedKmh, 0.5) // 8.0 m/s = 28.8 km/h
        assertTrue(calculator.stats.movingTimeMs > 0L, "Moving time must be credited")
