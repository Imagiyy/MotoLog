 ...
 *
 * Pure Kotlin, no Android imports — fully unit testable on JVM and iOS.
 *
 * Preservation rules:
...
object RouteDownsampler {
    ...
    private fun perpendicularDistance(
        ...
        val avgLat = toRadians((lineStart.latitude + lineEnd.latitude) / 2.0)
        val cosLat = cos(avgLat)

        val x = toRadians(point.longitude - lineStart.longitude) * cosLat * EARTH_RADIUS
        val y = toRadians(point.latitude - lineStart.latitude) * EARTH_RADIUS
        val x1 = 0.0
        val y1 = 0.0
        val x2 = toRadians(lineEnd.longitude - lineStart.longitude) * cosLat * EARTH_RADIUS
        val y2 = toRadians(lineEnd.latitude - lineStart.latitude) * EARTH_RADIUS
